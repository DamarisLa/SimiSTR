...
def main_reader(newFastaFile, newBedFile, oldBedfile):
	...
	print("successfull newBedfileRead)
	bedfile_l_old =getBedFile(oldBedfile)
	print("successfull oldBedFileRead)
...
if len(sys.argv) < 4:
	print("Please give a fastafile, the name and dir where the new dir has to be, the old bedfile,the  name and dir where the new bedfile should be, and a number between 0 and 1 indicating the change with which the original ref file")
else:
	for i in sys.argv:
        	print(i)
    	if os.path.isfile(sys.argv[1]) and os.path.isfile(sys.argv[3]):
		print("Entering main_reader")
